...
if __name__ == "__main__":
    ...
    parser.add_argument('files', type=lambda p: Path(p).resolve(strict=True), nargs='+', help="video files or directories containing video files for frame extraction ")
    ...
    for file in args.files:
        if file.is_file() and re.match(TV_FILENAME_RE, file.name):
            ...
            [files.append(f) for f in file.glob('*.mp4') if re.match(TV_FILENAME_RE, os.path.basename(f))]
        elif file.is_dir() and args.recursive:
            [files.append(f) for f in file.rglob('*.mp4') if re.match(TV_FILENAME_RE, os.path.basename(f))]

    if args.parallel:
        with mp.Pool(os.cpu_count()) as pool:
            tasks = [pool.apply_async(extract_frames_from_video, (file,),
                                      kwds={'fps': args.fps, 'overwrite': args.overwrite, 'prune': args.prune},
                                      callback=lambda f: print(f'{f.name} done')) for file in files]
            pool.close()
            pool.join()
