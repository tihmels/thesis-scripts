...
def evaluate(test_loader, model, epoch, tb_logger, loss_fun, args):
    ...
    with torch.no_grad():
        for idx, data in enumerate(test_loader):
            ...
            summary_ids = (
                score.detach().cpu().view(-1).topk(int(torch.count_nonzero(gt_summary)))[1]
            )
...
def TrainOneBatch(model, optimizer, scheduler, data, loss_fun, args):
    ...
    with torch.set_grad_enabled(True):
        ...
        print(f'Predicted Scores: {np.around(score.view(-1).detach().cpu().numpy()[::10], 3)}')
        print(f'GT Scores: {np.around(scores.detach().cpu().numpy()[::10], 3)}')
